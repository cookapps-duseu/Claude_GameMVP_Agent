...
`harness_state.md`의 `current_sprint == 1`이고 `visual_fidelity` 필드가 없거나 비어있으면:

1. `artifacts/GDD.md`의 `visual_assets` 값을 확인.
2. 아래 블록을 출력하고 사용자 입력을 기다린 뒤 `harness_state.md`에 `visual_fidelity: N`을 저장:
...
`visual_assets: false`이면 블록 아래에 추가 고지:
```
